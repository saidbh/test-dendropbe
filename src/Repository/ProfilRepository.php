...
//     */
    ...
    */
    ...
    public function findOneByUpdate(Profil $profil)
    {
        return $this->createQueryBuilder('p')
            ->andWhere('p.name = :val AND p.groupeType= :val2 AND p.id <>' . $profil->getId() . '')
            ->setParameter('val', $profil->getName())
            ->setParameter('val2', $profil->getGroupeType())
            ->getQuery()
            ->getResult();
    }
